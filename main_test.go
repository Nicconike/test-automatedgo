...
func TestGetGoVersionInfo(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			os:       "darwin",
			arch:     "amd64",
			wantURL:  "https://dl.google.com/go/go1.17.1.darwin-amd64.tar.gz",
			wantFile: "go1.17.1.darwin-amd64.tar.gz",
		},
		{
			name:     "Darwin ARM64",
			ver:      "go1.17.1",
...
func TestMain(m *testing.M) {
                    ...
                    "files": [
                        {
                            ...
                            "arch": "amd64",
                            "version": "go1.17.1",
                            "sha256": "dummychecksum123"
                        },
                        {
                            "filename": "go1.17.1.windows-amd64.zip",
                            "os": "windows",
                            "arch": "amd64",
                            "version": "go1.17.1",
                            "sha256": "dummychecksum456"
                        },
                        {
                            "filename": "go1.17.1.darwin-amd64.tar.gz",
                            "os": "darwin",
                            "arch": "amd64",
                            "version": "go1.17.1",
                            "sha256": "dummychecksum789"
                        },
                        {
                            "filename": "go1.17.1.darwin-arm64.tar.gz",
                            "os": "darwin",
                            "arch": "arm64",
                            "version": "go1.17.1",
                            "sha256": "dummychecksum101"
                        }
                    ]
                }
            ]`))
	...
	// Override the global URL variables
	originalVersionURL := VersionURL
	originalURL := URL
	VersionURL = mockServer.URL + "/VERSION"
	URL = mockServer.URL + "/dl/"
	defer func() {
		VersionURL = originalVersionURL
		URL = originalURL
	}()
	...
	for _, file := range expectedFiles {
		if _, err := os.Stat(file); os.IsNotExist(err) {
			log.Printf("Expected file %s was not created", file)
		} else {
			...
			if err != nil {
				...
				if err := json.Unmarshal(content, &info); err != nil {
					log.Printf("File %s does not contain valid JSON: %v", file, err)
				}
			}
		}
	}
